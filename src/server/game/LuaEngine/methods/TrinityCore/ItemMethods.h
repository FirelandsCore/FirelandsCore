...
namespace LuaItem
{
    ...
    {
        E->Push(item->IsRefundExpired(item->GetOwner()));
        return 1;
    }
    ...
    */
    ...
    {
        E->Push(item->GetTemplate()->GetName(DEFAULT_LOCALE));
        return 1;
    }
    ...
    */
    ...
    {
        E->Push(item->GetTemplate()->GetFlags());
        return 1;
    }
    ...
    */
    ...
    {
        E->Push(item->GetTemplate()->GetFlags2());
        return 1;
    }
    ...
    */
    ...
    int GetStatInfo(Eluna* E, Item* item)
    {
        ...
        if (statSlot > 0)
        {
            statValue = item->GetTemplate()->GetItemStatValue(statSlot);
            statType = item->GetTemplate()->GetItemStatType(statSlot);
        }
    ...
    {
        E->Push(item->GetTemplate()->GetArmorDamageModifier());
        return 1;
    }
    ...
    int SetRandomProperty(Eluna* E, Item* item)
    {
        ...
        ItemRandomEnchantmentType type = randomPropId > 0 ? ItemRandomEnchantmentType::Property : ItemRandomEnchantmentType::Suffix;
        uint32 id = randomPropId > 0 ? randomPropId : -randomPropId;
        
        ItemRandomEnchantmentId randomEnchantment(type, id);
        item->SetItemRandomProperties(randomEnchantment);
        return 0;
    }
    ...
    {
        uint32 randomSuffixId = E->CHECKVAL<uint32>(2);
        
        ItemRandomEnchantmentId randomEnchantment(ItemRandomEnchantmentType::Suffix, randomSuffixId);
        item->SetItemRandomProperties(randomEnchantment);
        return 0;
    }
