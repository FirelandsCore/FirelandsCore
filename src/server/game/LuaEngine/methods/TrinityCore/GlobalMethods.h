...
namespace LuaGlobalFunctions
{
    ...
    int RunCommand(Eluna* E)
    {
        ...
        eWorld->QueueCliCommand(new CliCommandHolder(nullptr, command, [](void*, char const*) {}, [](void*, bool) {}));
        return 0;
    }
    ...
    int PerformIngameSpawn(Eluna* E)
    {
        ...
        if (spawntype == 1) // spawn creature
        {
            if (save)
            {
                ...
                creature->SaveToDB(map->GetId(), phase);
            ...
            {
                TempSummon* creature = map->SummonCreature(entry, pos);
                if (!creature)
                {
                    E->Push();
                    return 1;
        ...
        if (spawntype == 2) // Spawn object
        {
            ...
            if (save)
            {
                ...
                object->SaveToDB(map->GetId(), phase);
